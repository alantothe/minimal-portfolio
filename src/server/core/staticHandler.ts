...
export class StaticHandler {
  ...
  isStaticRequest(pathname: string): boolean {
    return (
      pathname.startsWith(serverConfig.static.publicPath) ||
      pathname.startsWith(serverConfig.static.pagesPath) ||
      pathname.startsWith(serverConfig.static.layoutPath) ||
      serverConfig.static.allowedExtensions.some((ext: string) =>
   ...
   *
   * The mapping itself lives in `staticPath.ts` so the content importer can
   * apply the identical rule when it uploads these same files.
   */
  private resolveFilePath(pathname: string): string {
    return resolveStaticFilePath(pathname);
  }
  ...
  async handleStaticRequest(request: Request): Promise<Response> {
    try {
      ...
      const serverError = new ServerError(
        "An unexpected error occurred while serving static file"
      );
