...
import { createSeoMetadata, type SeoPageInput } from "../services/seo";
...
async function loadPageData(pageName: string): Promise<any> {
  if (pageName === "home") {
    try {
      const [githubCommits, githubActivity, blogPostCount, totalViews] =
        await Promise.all([
          getMonthlyCommitCount(),
          getYearlyContributionActivity(),
          countBlogPosts(),
      ...
      const githubUsername =
        process.env.GITHUB_USERNAME || homeConfig.author.githubUsername;
      ...
      return {
        ...
        metrics: {
          ...
          githubCommits:
            githubCommits > 0
              ? githubCommits
              : homeConfig.metrics.githubCommits,
          githubActivityPanel: renderGitHubActivityPanel(
            githubActivity,
            githubUsername
          ),
...
export function createApiHandler(url: URL) {
  return async (): Promise<Response> => {
    try {
      ...
      const seoInput: SeoPageInput =
        pageKind === "blog" || pageKind === "projects"
          ? { kind: pageKind, page: pageNumber }
          : { kind: pageKind };
      const seo = createSeoMetadata(seoInput, url);

      return new Response(JSON.stringify({ ...pageData, seo }), {
        headers: {
