...
describe("public HTTP behavior", () => {
  ...
  test("project listing exposes every selected project through anchors", async () => {
    ...
    expect(firstHtml).not.toContain('data-previewable="true"');
    expect(firstHtml).not.toContain('class="project-image__tooltip"');
    expect(firstHtml).not.toContain('aria-describedby="project-image-tooltip-');
